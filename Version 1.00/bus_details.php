...
$routeID = $routeRow['RouteID'] ?? null;
mysqli_stmt_close($stmt);

$gateData = [];
if ($routeID) {
    // Step 2: Fetch GateIDs from route_gate to rely on database insertion order
    $rgQuery = "SELECT GateID FROM route_gate WHERE RouteID = ?";
...
<body class="bg-gray-50 min-h-screen font-sans antialiased">
    ...
    <h2 class="text-2xl font-semibold text-gray-800 mb-4 border-b-2 border-blue-500 pb-2 flex items-center"> <img src="assets/images/SVG/bus_stop.svg" alt="Bus Stop Icon" class="w-10 h-11 mr-2">
        Bus Stops
    </h2>
        ...
        <?php if (!empty($gateData)): ?>
            ...
            <?php foreach ($gateData as $gate): ?>
                <a href="gate_details.php?GateID=<?php echo htmlspecialchars($gate['GateID']); ?>" 
                   class="block bus-stop-card bg-white rounded-xl p-4 flex flex-col justify-between border border-gray-300 shadow-md transition-all duration-300 hover:shadow-xl">
            ...
            try {
                ...
                if (gates && gates.length > 0) {
                    ...
                    coords.forEach(({ gate, lat, lng }) => {
                        const marker = L.marker([lat, lng], { icon: gateIcon, title: gate.GateName }).addTo(map);
                    ...
                    const drawStraightLine = () => {
                        ...
                        if (path.length >= 2) {
                            // Casing + main line (navigation style)
                    ...
                    buildRoadRoute()
                        ...
                        .catch(() => {
                            // Keep the straight line if routing fails.
                        });
